...
async function ProfileLayout({ children, params: { username } }) {
  const profile = await fetchProfile(username);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex gap-x-5 md:gap-x-10 px-4">
          <ProfileAvatar user={profile}>
            <UserAvatar user={profile} className="w-20 h-20 md:w-36 md:h-36 cursor-pointer" />
          </ProfileAvatar>
          ...
          <div className="md:px-10 space-y-4">
            ...
            <div className="flex items-center gap-x-7">
              ...
              <Link href={`/dashboard/${profile.username}/followers`} className="font-medium">
                <strong>{profile.followedBy.length}</strong> followers
              </Link>

              <Link href={`/dashboard/${profile.username}/following`} className="font-medium">
